...
class Character{
    ...
    //static methods are methods that are associated with the class itself, rather than with any particular instance of the class. 
...
class Adventurer extends Character {
    ...
    check(){
        let roles=["Fighter", "Healer", "Wizard"];
    if (roles.includes(this.role)) {console.log(`${this.role} is included in ${roles}`)}
    if (! roles.includes(this.role)) throw new Error(console.log(`${this.role} is not included in ${roles}, you need to try another role`))
   }
...
robin.companion.companion.inventory = ["small hat", "sunglasses"];
    ...
    class AdventurerFactory {  
      ...
      const newRobin = healers.generate("newRobin");
      console.log(healers)
